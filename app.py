...
ax.plot(data.index[time_step:train_size+time_step], y_train_inv, label='Train Actual')
ax.plot(data.index[time_step:train_size+time_step], train_predict_inv, label='Train Predicted')
ax.plot(data.index[train_size+time_step:], y_test_inv, label='Test Actual')
ax.plot(data.index[train_size+time_step:], test_predict_inv, label='Test Predicted')
ax.set_xlabel('Date')
...
ax.plot(days_before, last_30_days, label='Last 30 Days (Actual)', color='blue')

# Plot the predicted 10 days
days_ahead = np.arange(len(predicted_next))
ax.plot(days_ahead, predicted_next, label='Next 10 Days (Predicted)', color='red', linestyle='--')

ax.axvline(x=0, color='gray', linestyle='-', alpha=0.5)
ax.set_xlabel('Days (0 = Today)')
ax.set_ylabel('Stock Price')
ax.set_title(f'{stock_symbol} - 10 Day Price Prediction')
ax.legend()
